...
-------------------------------------------------------------
  ...
  subtype contador_t_3 is integer range 0 to 10;
  signal count3: contador_t_3;
  ...
  signal ctr_we_mem     : std_logic; --Controla el write enable de la memoria
  signal ctr_we_mem_vector: std_logic_vector(0 downto 0);
  -- SENNALES DE RELOJ
  ...
  signal counter3 : std_logic_vector(3 downto 0);--Salida del contador 3
  ...
  signal salida_memoria:  std_logic_vector(3 downto 0);
  ...
  signal mod_mem_terminado: std_logic;--Indica si la secuencia de victoria/derrota ha terminado
begin
 -----------------------------------------------------------------------------
...
-------------------------------------------------------------
  (ctr_dig, ctr_count_en,ctr_leds1,ctr_leds2,ctr_we_mem) <= ctrl;
  status <= (iguales,sec_terminada,mod_mem_terminado);
  digito_1 <= dig1;
  digito_2 <= dig2;
  cero <= (others=>'0');
  cero_write <= (others=>'0');
  leds<=leds_int;
  salida_mem <= salida_memoria;
  ctr_we_mem_vector(0)<=ctr_we_mem;
-------------------------------------------------------------
-- DIVISOR
-------------------------------------------------------------
...
p_count3: process(clk_frec_0, rst)
begin
  if rst = '1' then
    count3 <= 0;
  elsif rising_edge(clk_frec_0) then
      if ctr_we_mem = '1' then
        count3 <= count3 +1;
      end if;
  end if;
end process p_count3;
...
begin
  ...
  salida_memo: process(mem1,ctr_we_mem) is
  begin
    if ctr_we_mem='1' then
        salida_memoria<=mem1;
    else
        salida_memoria<=(others=>'0');
    end if;
end process salida_memo;
-------------------------------------------------------------
-- MEMORIAS
-------------------------------------------------------------
