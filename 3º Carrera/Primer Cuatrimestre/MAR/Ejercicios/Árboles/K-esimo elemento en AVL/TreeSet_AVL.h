...
protected:
	T const& kesimo_imp(int k, Link & a) const {
		if (a == nullptr) {
			throw std::domain_error("ERROR");
		}
		else if (a->tam_i == k) {
			...
			return kesimo_imp(k - a->tam_i, a->dr);
		}
		else {//a->tam_i > k
			return kesimo_imp(k, a->iz);
		}
	}
   ...
   void rotaDer(Link & r2) {
	  ...
	  //r1->tam_i = r1->tam_i; porque el hijo izquierdo del arbol r1 no lo tocamos
	  // La antigua r2->tam_i es 1 + r1->tam_i + #nodos(r1->dr_antiguo)
	  // Ahora r2->iz = r1->dr_antiguo por lo que 
	  // r2->tam_i = 1 + #nodos(r2->iz) = 1 + #nodos(r1->dr_antiguo) = r2->tam_i - r1->tam_i
   ...
   void rotaIzq(Link & r1) {
      ...
      r2->altura = std::max(altura(r2->iz), altura(r2->dr)) + 1;
	  //r1->tam_i = r1->tam_i; porque el hijo izquierdo del arbol r1 no lo tocamos
	  // La antigua r2->tam_i es 1 + #nodos(r2->iz_antiguo)
	  // Ahora r2->iz = arbol(r1->iz_antiguo, r1, r2->iz_antiguo) por lo que 
	  // r2->tam_i = 1 + #nodos(r2->iz) = 1 + #nodos(r1->iz_antiguo) + 1 + #nodos(r2->iz_antiguo) = r1->tam_i + r2->tam_i
	  r2->tam_i = r2->tam_i + r1->tam_i;
      r1 = r2;
   }
