...
const bool VerModoEMaquina = true;/*Cuando esta constante está a true, al jugar en el modoE en pantalla aparecerá la desviación típica del humano, 
...
void modoEmaquina(tConjuntoCartas& mazo, tConjuntoCartas& cartasMaquina, float& puntos, const tPosiblesCartas& tabla) {
	...
	if (VerModoEMaquina) {
		cout << "Desviacion típica (Consistencia del jugador): " << desviacionTipica(xi, fi) << endl;
	}
	...
	if (desviacionTipica(xi, fi) < DesviacionTipica) {		//Jugador consistente
		...
		if (estimacionPuntuacionHumano(tabla, robadasHumano) > SyM) {
			if (VerModoEMaquina) {
				cout << estimacionPuntuacionHumano(tabla, robadasHumano) << " puntos. La estimación no es real por lo que la máquina se plantará al alcanzar la puntuación arbitraria de " << PuntuacionArbitraria << " puntos." << endl << endl;
			}
