/*Juan Carlos Llamas Núñez y Adrián Sanjuán Espejo  Grupo: Doble Grado*/
...
//Parte Opcional
const bool VerModoEMaquina = false;/*Cuando esta constante está a true, al jugar en el modoE en pantalla aparecerá la desviación típica del humano (inconsistencia), 
								   la estimación de la máquina y, al final de la partida, la tabla de las decisiones que ha tomado el jugador humano.
								   Es útil ponerla a true para ver cómo razona la máquina en el modoE*/
//Fin Parte Opcional				
const int Palos = 4;
const int Figuras = 3;
...
//Parte Opcional
const float DesviacionTipica = 1.2;
...
typedef tPosiblesPuntos tPosiblesCartas[CartasPosibles]; /*Los tres tipos anteriores están diseñados para definir una tabla cuyas columnas
														 representan los posibles puntos con los que se puede plantar el humano, y las filas
														 el número posible de cartas con el que se puede plantar. En cada coordenada se guardan
														 tanto las oportunidades que ha tenido para plantarse con ese número de cartas y esos puntos como las 
														 veces que realmente lo ha hecho*/
typedef double fila[PuntosPosibles];
//Fin Parte Opcional

int menu();//Devuelve un entero en el rango [0,5] dependiendo del modo de juego elegido.

void modoAcuerpo();//Las funciones modoXcuerpo son procedimientos que se incluyen únicamente para aliviar la carga de código del main correspondiente al modoX.
...
float modoBmaquina(ifstream& file, int numCartas, float puntosHumano);/*Recibe el mazo y el número máximo de cartas que se pueden robar y
recibe la puntuación del jugador humano. Devuelve la puntuación obtenida por la máquina, robará si su puntuación es menor o igual a la del humano.*/

void modoCcuerpo();
void modoChumano(ifstream& file, tCartasPorAparecer cartas, float& puntos);/*Recibe el mazo, el array que contabiliza las cartas que quedan por salir 
y la puntuación del humano. Modifica el array de las cartas que quedan por salir y la puntuación del humano.*/
void modoCmaquina(ifstream& file, tCartasPorAparecer cartas, float puntosHumano, float& puntos);/*Recibe el mazo, el array que contabiliza las cartas 
que quedan por salir y las puntuaciones del humano y la máquina. Modifica el array de las cartas que quedan por salir y la puntuación de la máquina.*/
bool esProbablePasarse(float puntosMaquina, const tCartasPorAparecer cartas);/*Recibe la puntuación de la máquina y el array con las cartas por aparecer. 
Devuelve true si la probabilidad de pasarse al robar una carta excede 0.5 y false en caso contrario.*/

int determinaGanador(float puntosJugador, float puntosMaquina);/*Recibe la puntuación del humano y de la máquina. 
															   Devuelve un 2 si gana la máquina y un 1 si gana el humano.*/

void modoDcuerpo(int partidasJugadas);
void inicializa(tConjuntoCartas& cartas);// Inicializa el contador cartasRobadas a 0 (lista de cartas vacía).
...
void modoDhumano(tConjuntoCartas& mazo, tCartasPorAparecer	cartas, tConjuntoCartas& cartasHumano, float & puntos);/*Recibe el mazo, 
el array que contabiliza las cartas que quedan por salir, la puntuación del humano y la lista de cartas robadas por el humano. Modifica el mazo, 
las cartas que quedan por salir, la puntuación del humano y las cartas robadas por el humano.*/
...
contabiliza las cartas que quedan por salir, la puntuación del humano y de la máquina y la lista de cartas robadas por el humano. Modifica el mazo, 
las cartas que quedan por salir, la puntuación de la maquina y las cartas robadas por la máquina.*/
...
robadas por el humano y la máquina y también su puntación, asi como el número de partidas de la ejecución correspondiente. Es la encargada de decidir el ganador y escribir en un 
archivo de salida los resultados de la partida.*/
int determinaGanadorModoD(float puntosJugador, float puntosMaquina, int robadasHumano, int robadasMaquina);/*Recibe la puntuación del humano y de la máquina y
el numero de cartas que han robado ambos. Devuelve un 2 si gana la máquina y un 1 si gana el humano. (Como en el modo D para decidir el ganador no es suficiente 
con recibir las puntuaciones del jugador y de la máquina, y al ser "determinaGanador" una función exigida con unos determinados parámetros, hemos preferido crear 
una función específica antes que añadirle otros parámetros "determinaGanador" */
...
float valor(int card);//Recibe una carta (entero en [1,7]U[10,12]) y devuelve el valor o puntuación de la carta (0.5 si es una figura y el mismo que el de la carta en caso contrario)
...
//Parte Opcional (los comentarios son un poco más extensos con el objetivo de explicar este modo, el cual no tiene enunciado.)
/*Tras numerosas pruebas hemos comprobado que hasta las 20-30 partidas, la máquina no tiene suficiente información 
para ganar la mayoría de veces.*/
//---------------------------------------------------------------------------------------------------------------------------
...
Modifica el mazo, la puntuación del humano y las cartas robadas por el humano.*/
...
puntuaciones muy bajas (0.5, 1, 1.5) y otras veces con puntuaciones muy altas(6, 6.5, 7), aunque la media de las puntuaciones entorno a las cuales se planta esté próxima a 4, la desviación típica 
...
Hemos considerado que un jugador es inconsistente con sus decisiones cuando la desviación típica es mayor que 1.2 (valor definido como la constante "DesviacionTipica").*/
...
int main() {
	...
	if (VerModoEMaquina) {
		...
		for (int i = 0; i < CartasPosibles; i++) {
			cout << endl << setw(2) << right << i + 1 << setw(3) << "   " << left;
			for (int j = 0; j < PuntosPosibles; j++) {
				cout << setw(2) << tabla[i][j].plantado << "/" << setw(2) << left << tabla[i][j].total << right << " ";
			}
		}
...
void modoEmaquina(tConjuntoCartas& mazo, tConjuntoCartas& cartasMaquina, float& puntos, const tPosiblesCartas& tabla) {
	...
	if (VerModoEMaquina) {
		cout << "Desviacion típica (Inconsistencia del jugador): " << desviacionTipica(xi, fi) << endl;
	}
	...
	if (desviacionTipica(xi, fi) < DesviacionTipica) {		//Jugador consistente
		...
		if (estimacionPuntuacionHumano(tabla, robadasHumano) > SyM) {
			if (VerModoEMaquina) {
				cout << estimacionPuntuacionHumano(tabla, robadasHumano) << " puntos. La estimación no es real por lo que la máquina se plantará al alcanzar la puntuación " << endl << "arbitraria de " << PuntuacionArbitraria << " puntos." << endl << endl;
			}
		...
		if (robadasHumano * puntuacionMediaPorCartaSacada() > SyM) {
			if (VerModoEMaquina) {
				cout << " La estimacion no es real por lo que la máquina se plantará al alcanzar la puntuación " << endl << "arbitraria de " << PuntuacionArbitraria << " puntos." << endl << endl;
			}
			while (puntos < PuntuacionArbitraria) {
				robarmodoE(mazo, cartasMaquina, puntos);
				if (puntos < PuntuacionArbitraria) {
...
float estimacionPuntuacionHumano(const tPosiblesCartas& tabla, int robadas) {
	...
	if (estimacion == 0) { //e.d. no se ha plantado nunca con "robadas" cartas
		...
		final = estimacion / ponderacion;/*estimamos que la puntuación con la que se ha plantado ha sido la media ponderada de las puntuaciones con las que se ha plantado en 
											el pasado cuando se ha plantado con "robadas " cartas. Se le da más peso a las puntuaciónes con las que el jugador ha sido más consistente al plantarse.
