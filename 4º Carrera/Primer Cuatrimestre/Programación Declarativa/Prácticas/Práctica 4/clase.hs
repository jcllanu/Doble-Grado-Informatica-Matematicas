...
muevebis :: Punto -> Direccion -> Punto
muevebis p Arriba = if f>=1 then (f-1,c) else p where (f,c) = p
muevebis p Abajo = if f<=99 then (f+1,c) else p where (f,c) = p
muevebis p Izquierda = if c>=1 then (f,c-1) else p where (f,c) = p
muevebis p Derecha = if c<=99 then (f,c+1) else p where (f,c) = p
...
destinobis :: Punto -> [Direccion] -> Punto
destinobis p ms = foldl muevebis p ms
...
trayectoriabis :: Punto -> [Direccion] -> [Punto]
trayectoriabis p ms = foldl (\xs y -> xs++[muevebis (last xs) y]) [p] ms
--2
