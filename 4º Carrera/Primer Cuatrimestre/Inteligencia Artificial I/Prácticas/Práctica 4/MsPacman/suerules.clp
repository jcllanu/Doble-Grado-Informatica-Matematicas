...
(defrule SUEsepararseBlinky
    (declare (salience 10))
	(SUE (edible false) (distanceNearestGhost ?d) (distancePacman ?p) (nearestGhost BLINKY))
	(BLINKY (edible false))
	(test (< ?d 20)) (test (< ?d ?p)) (test (> ?d 0)) (test (> ?p 0))
...
(defrule SUEsepararseInky
	...
	(INKY (edible false)) (test (> ?d 0)) (test (> ?p 0))
	(test (< ?d 20)) (test (< ?d ?p))
	=>
	(assert (ACTION (id SUErunAwayFromGhost) (info "Inky esta muy cerca --> separarse de el") (ghost INKY)))
)

;Cuando solo queda 1 PowerPill, SUE ayuda a INKY a defenderla
