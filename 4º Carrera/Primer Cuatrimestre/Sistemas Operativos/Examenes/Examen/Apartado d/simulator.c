...
int main(int argc, char *argv[]) {
	...
	//Con dos argumentos el segundo es el nombre del fichero donde estan almacenados los nombres de las paradas
	//Tiene que contener una linea por parada
	if(argc==2){
		flag=1;
	}else if(argc==1){
		flag=0;
	}else{
		printf("Uso 1: ./simulator <archivo_paradas>\n");
		printf("Uso 2: ./simulator\n");
		printf("Ejemplo: ./simulator paradas.txt\n");
		return 1;
	}
	if(flag){//FLAG
		flag=1;
		...
		for(int i=0; i<N_PARADAS;i++){
			...
			char* strg_ptr = malloc((tamanno-1)*sizeof(char));
			if(fseek(file,-tamanno,SEEK_CUR)==-1){
				return NULL;
			}
			if(tamanno-1!=fread(strg_ptr,sizeof(char),tamanno-1,file)){
				return NULL;
			}
			fseek(file,1,SEEK_CUR);
			nombre_paradas[i]=strg_ptr;
		}
		/*for(int i=0; i<N_PARADAS; i++){
			printf("%s\n",nombre_paradas[i]);
		}*/
	...
	//FREES
	if(flag){
		for(int j=0; j<N_PARADAS;j++){
			free(nombre_paradas[j]);
		}
	}
	return 0;
}
...
void Autobus_En_Parada(){
	...
	estado = EN_PARADA;
	if(flag){
		printf("El autobus llega a la parada %s\n" , nombre_paradas[parada_actual]);
	}else{
	...
	if(flag){
		printf("El autobus sale de la parada %s\n" , nombre_paradas[parada_actual]);
	}else{
...
void Subir_Autobus(int id_usuario, int prio, int origen){
	...
	if(flag){
		printf("usuario %d (%s):El usuario %d quiere subir en la parada %s\n" ,id_usuario, prioridad, id_usuario, nombre_paradas[origen]);
	}else{
	...
	if(prio==USUARIOS_PRIO){
		prioritarios_esperando[origen]++;
		...
		while(parada_actual!=origen ||estado!=EN_PARADA || n_ocupantes==MAX_USUARIOS || prioritarios_esperando[origen]>0){
			//Annadimos la condicion de que no haya prioritarios esperando
			pthread_cond_wait(&b_en_parada,&mutex);
		}
	}
	if(flag){
		printf("usuario %d (%s):El usuario %d sube al autobus en la parada %s\n" ,id_usuario, prioridad, id_usuario, nombre_paradas[origen]);
	}else{
	...
	if(prio==USUARIOS_PRIO){
		prioritarios_esperando[origen]--;
		//Avisa a los demás que se ha montado (pueden ser no prioritarios y estar durmiendo)
		if(esperando_parada[origen]>0 && prioritarios_esperando[origen]==0){//Soy el último prioritario en subir
			pthread_cond_broadcast(&b_en_parada);
		}
	}
...
void Bajar_Autobus(int id_usuario, int prio, int destino){
	...
	pthread_mutex_lock(&mutex);
	if(flag){
		printf("usuario %d (%s):El usuario %d quiere bajarse en la parada %s\n" ,id_usuario, prioridad, id_usuario, nombre_paradas[destino]);
	}else{
	...
	esperando_bajar[destino]--;
	if(flag){
		printf("usuario %d (%s):El usuario %d se baja en la parada %s\n" ,id_usuario, prioridad, id_usuario, nombre_paradas[destino]);
	}else{
