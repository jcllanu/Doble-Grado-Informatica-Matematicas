...
public:
	...
	std::pair<bool, T> lower_bound(T const& e) const {
		auto dev= cota_inferior(e, raiz);
		return dev;
		
	}
protected:

	std::pair<bool, T> cota_inferior(T const& e, Link a) const {
		if (a== nullptr) {
			std::pair<bool, T> dev;
			dev.first = false;
			return dev;
			...
			return std::make_pair(true, a->elem);
		}
		else if (menor(e, a->elem)) {
			auto sol = cota_inferior(e, a->iz);
			if (!sol.first) {
				sol = std::make_pair(true, a->elem);
			}
			return sol;
		}
		else {
			return cota_inferior(e, a->dr);
		}
