...
public:
	...
	int count_interval(const T &lower, const T &upper) const {
		return contar_intervalo(lower, upper, raiz);
	}
protected:
	//NUEVO
	int contar_intervalo(const T &lower, const T &upper, Link a) const {
		if (a== nullptr) {
			return 0;
		}
		else {
			...
			if (a->elem > lower) {
				numIzq = contar_intervalo(lower, upper, a->iz);
			}
			if (a->elem < upper) {
				numDer = contar_intervalo(lower, upper, a->dr);
			}
			if (a->elem >= lower && a->elem <= upper) {
				numIzq++;
			}
