...
public:
	...
	protected:
		...
		// construye el iterador al nodo con el elemento clave
		const_iterator(const T &clave, Link raiz, Comparator menor) {
			//raiz es distinto de nullptr
			if (raiz->elem == clave) {
				act = raiz;
			}
			else if (menor(clave, raiz->elem)) {
				if (raiz->iz != nullptr) {
					ancestros.push(raiz);
					std::stack<Link> hastaAhora = ancestros;
					*this = const_iterator(clave, raiz->iz, menor);
					std::stack<Link> nuevos;
					while(!ancestros.empty()) {
						nuevos.push(ancestros.top());
						ancestros.pop();
					}
					ancestros = hastaAhora;
					while (!nuevos.empty()) {
						ancestros.push(nuevos.top());
						nuevos.pop();
					}
				...
				if (raiz->dr != nullptr) {
					std::stack<Link> hastaAhora = ancestros;
					*this = const_iterator(clave, raiz->dr, menor);
					std::stack<Link> nuevos;
					while (!ancestros.empty()) {
						nuevos.push(ancestros.top());
							ancestros.pop();
	...
	const_iterator busca(const T &clave) const {
		if (raiz == nullptr) {
			return end();
		}
		return const_iterator(clave, raiz, menor);
	}
};
