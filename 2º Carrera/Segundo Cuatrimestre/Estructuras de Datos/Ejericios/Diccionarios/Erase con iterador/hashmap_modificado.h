...
template <class Clave, class Valor,
	...
	protected:
		...
		protected:
			...
			Iterador(pointer_tabla t,  Iterador const& it) : tabla(t) {
				if (it.act == nullptr|| it.ind== tabla->array.size()) {
					throw std::out_of_range("No hay elemento a borrar");
				}
				
				Link pos = tabla->array[it.ind];
				Link ant = nullptr;
				bool encontrado = false;
				Clave c = it.act->cv.clave;
				while (!encontrado && pos != nullptr) {
					
					if (tabla->pred(c, pos->cv.clave))
						encontrado = true;
					else {
						ant = pos;
						pos = pos->sig;
				...
				if (pos==nullptr) {//No se ha encontrado el elemento a borrar
					ind = tabla->array.size();
					act = nullptr;
				}
				else {
					if (ant == nullptr) {//El elemento a borrar es el primero de la lista
						tabla->array[it.ind] = pos->sig;
					}
					else {
						ant->sig = pos->sig;
					}
