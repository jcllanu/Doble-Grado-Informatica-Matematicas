...
public class ExplosiveAlien extends AlienShip{
	...
	public void onDelete() {
		...
		for(int i=-1; i<2; i++) {
			for(int j=-1; j<2; j++) {
				if(i!= 0||j!=0) {
					if(this.getGame().getListaObjetos().getObjectInPosition(getFila()+i,columna+j)!=null) {
						this.performAttack(this.getGame().getListaObjetos().getObjectInPosition(getFila()+i,columna+j));
					}
