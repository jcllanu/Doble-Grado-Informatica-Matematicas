...
public class GameObjectBoard {
	...
	public boolean rotate(boolean derecha){
		boolean ok=true;
		for (int i=0; ok && i<contador; i++) {
			if (derecha) {
				objects[i].setFil(objects[i].getCol());
				objects[i].setCol(Game.NUM_FILAS-1-objects[i].getFil());
				
			}else {
				objects[i].setFil(Game.NUM_COLUMNAS-1-objects[i].getCol());
				objects[i].setCol(objects[i].getFil());
				
			}
			
			if(objects[i].isOut()&& !(objects[i] instanceof Ovni)) {
				ok=false;
			}
		}
		return ok;
