...
public class DestroyerAlienList {
	...
	public String characterAtToString(int i, int j) {//Devuelve el string del destroyer en la posición i,j o el caracter vacío si no hay destroyer
	...
	public int impactaLaser(UCMShipLaser laser) { //Gestiona los impactos entre laser y destroyers
		int puntos=-1;
	...
	private void eliminar(int i) { //Elimina el destroyer i de la lista de destroyers
		for(int j=i+1; j<this.getContador(); j++) {
			this.setListaDestroyerAlien(this.getListaDestroyerAlien(j),j-1);
		}
		this.setContador(this.getContador()-1);
	}
	public int shockWave() { //Quita un punto de resistencia a todos los destroyers y los elimina en caso necesario
