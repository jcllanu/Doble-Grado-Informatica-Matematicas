...
public class Game implements IPlayerController{
	...
	public String data() {
		...
		text+="\nHyperspace: "+this.getNave().getHyperspace().toString();/*Modificado*/
		return text;
	}
	...
	@Override
	public void move(int numCells) throws OffWorldException, HyperspaceException, WarmUpEngineException {/*IGUAL HAY QUE IMPLEMENTAR ESTO EN UCMShip*/
		if(nave.getHyperspace()==Mode.WARM_UP) {
			nave.setHyperspace(Mode.ON);
			throw new WarmUpEngineException("You can't move because engine is warming up");
		}
		if(!(nave.getCol() + numCells < 0 || nave.getCol() + numCells >=NUM_COLUMNAS)) {
			if(numCells>2||numCells<-2) {
				if(this.getNave().getHyperspace()!=Mode.ON) {
					throw new HyperspaceException("You can't move that far without the hypersepace mode.");
				}else {
					nave.setCol(nave.getCol() + numCells);
					nave.setHyperspace(Mode.OFF);
				}
			}else {
				nave.setCol(nave.getCol() + numCells);
			}
			...
			if((numCells>2||numCells<-2)&&nave.getHyperspace()==Mode.ON) {
				nave.setHyperspace(Mode.OFF);
			}
	...
	public String serialize() {
		...
		dev+="\nHYP;"+nave.getHyperspace().toString();/*Serializado del hyperspace*/
	...
	public void hyperspace() throws HyperspaceException {
		if(this.getNave().getHyperspace()==Mode.OFF) {
			this.getNave().setHyperspace(Mode.WARM_UP);
		}else{
			throw new HyperspaceException("You already have enabled the hyperspace");
		}
	}
